...
public class AddressController {
    ...
    /* The method handles Address save Related request. It takes the details as per in the SaveAddressRequest
      ...
      */
    @CrossOrigin
    @RequestMapping(method = RequestMethod.POST, path = "/address", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<SaveAddressResponse> saveAddress(@RequestHeader("authorization") final String authorization, @RequestBody SaveAddressRequest saveAddressRequest) throws AuthorizationFailedException, AddressNotFoundException, SaveAddressException {
    ...
    public ResponseEntity<AddressListResponse> getAllSavedAddress(@RequestHeader("authorization") final String authorization) throws AuthorizationFailedException {
        ...
        // Calls customerService getCustomer Method to check the validity of the customer. This methods returns the customerEntity to get all address.
        CustomerEntity customerEntity = customerService.getCustomer(accessToken);
  ...
  & produces response in DeleteAddressResponse and returns UUID of deleted address and Successful message. If error Return error code and error Message.
   */
    @CrossOrigin
    @RequestMapping(method = RequestMethod.DELETE, path = "/address/{address_id}", produces = MediaType.APPLICATION_JSON_VALUE)
   ...
   */
    ...
    public ResponseEntity<StatesListResponse> getAllStates() {
        ...
        if (!stateEntities.isEmpty()) {// Checking if StateEntities is empty.
            ...
            // looping in to get details of all the stateEntity & then create a stateList and add UUID of state and stateName and add the newly created StateList to the list of StateList.
